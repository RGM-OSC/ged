...
sub write_event_host_to_rss ($) {
   ...
   while (my $event = $Ged_Request->fetchrow_hashref( 'NAME_lc' )) {
      print "write_event_host_to_rss -> contract: ", $event->{ contract }, " equipment: ", $event->{ equipment }, " service: ", $event->{ service }, " description: ", $event->{ description }, "\n" if $verbose;
      print FileRSS "<item>\n";
      print FileRSS "<title>", $event->{ contract } ,": Asset->", $event->{ equipment }," service:",$event->{ service },"</title>\n";
      print FileRSS "<description>", $event->{ description } ,"</description>\n";

      if ( $event->{ service } eq "HOSTDOWN" || $event->{ service } eq "HOST UNREACHABLE" )
      {
        print FileRSS "<link>https://$rgm_Host/thruk/cgi-bin/extinfo.cgi?type=1&amp;host=",$event->{ equipment },"</link>\n";
      }
      else
      {
        print FileRSS "<link>https://$rgm_Host/thruk/cgi-bin/extinfo.cgi?type=2&amp;host=",$event->{ equipment },"&amp;service=",$event->{ service },"</link>\n";
      }
      print FileRSS "</item>\n";
   }
}
...
sub set_filter_type($;$) {
    ...
    { 
        ...
        {
            $MainType="contract";
            print "set_filter_type -> MainType = contract. \n" if $verbose;
        }
...
my $Ged_Request = $db_ged->prepare('select id,contract, equipment,service,description from nagios_queue_active;');
$Ged_Request->execute();
                ...
                foreach $e (@{$CurrentFilterXMLFile->{filters}})
                        {
                                          ...
                                          if(ref($e->{filter}) eq 'ARRAY') # Multi-Filter user, and multi-condition filter.
                                          {
                                               ...
                                               foreach $filter (@{$e->{filter}}) {
                                                    ...
                                                    # ACT Now
                                                    if ( $MainType eq 'contract' && ( not $MainType eq 'invalid'))
                                                    {
							 print "Main -> Entering multi-filter multi-user. Current type : equipment ($setval)\n" if $verbose;
                                                         my $Ged_Request = $db_ged->prepare('select id,contract, equipment,service,description from nagios_queue_active where contract like \''. $setval .'\' ;');
                                                         $Ged_Request->execute();
                                                    ...
                                                    if ( $MainType eq 'host' && ( not $MainType eq 'invalid'))
                                                    {
                                                      ...
                                                      {
                                                            print "Main -> Entering multi-filter multi-user. Current type : equipment ($setval)\n" if $verbose;                                                          
                                                            my $Ged_Request = $db_ged->prepare('select id,contract, equipment,service,description from nagios_queue_active where equipment like \''. $setval .'\' ;');
                                                            $Ged_Request->execute();
                                                      ...
                                                      else ###### At this point, if this is not an equipement filter, it can only be hostgroup.  
                                                      {
                                                        ...
                                                        ##### Now XML to RSS File
                                                        my $Ged_Filtered_Request = 'select id,contract, equipment,service,description from nagios_queue_active where equipment like \''. $HostList[0] .'\' ';
                                                        foreach (@HostList) 
                                                        {
                                                          print "Main ->          HOST: $_ \n" if $verbose;
                                                    ...
                                                    if ( $MainType eq 'service' && ( not $MainType eq 'invalid'))
                                                    {
                                                      ...
                                                      if ( defined($type) && $type eq 'service' ) # Is this an equipement ?
                                                      {
                                                            ...
                                                            my $Ged_Request = $db_ged->prepare('select id,contract, equipment,service,description from nagios_queue_active where service like \''. $setval .'\' ;');
                                                            $Ged_Request->execute();
                                                      ...
                                                      else ###### At this point, if this is not an service filter, it can only be servicegroup.  
                                                      {
                                                        ...
                                                        ##### Now XML to RSS File
                                                        my $Ged_Filtered_Request = 'select id,contract, equipment,service,description from nagios_queue_active where service like \''. $ServiceList[0] .'\' ';
                                                        foreach (@ServiceList) 
                                                        {
                                                    ...
                                                    if ( $MainType eq 'description' )
                                                    {
                                                      ...
                                                      print "Main -> Entering multi-filter multi-user. Current type : service ($setval)\n" if $verbose;                                                          
                                                      my $Ged_Request = $db_ged->prepare('select id,contract, equipment,service,description from nagios_queue_active where description like \''. $setval .'\' ;');
                                                      $Ged_Request->execute();
